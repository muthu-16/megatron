"""
dashboard/server.py — MEGATRON Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "MEGATRON Uploads",
        Path.home() / "Documents" / "MEGATRON Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'MEGATRON-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"MEGATRON Dashboard Port {port}"
        prog_rule  = "MEGATRON Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="MEGATRON_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until MEGATRON is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "MEGATRON.key").exists() and (certs / "MEGATRON.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in MEGATRON to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('MEGATRON_token','{tok}');
  sessionStorage.setItem('MEGATRON_key','{key}');
  localStorage.setItem('MEGATRON_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to MEGATRON…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "MEGATRON.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "MEGATRON.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in MEGATRON UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
